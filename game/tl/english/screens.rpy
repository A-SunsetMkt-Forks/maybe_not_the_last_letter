# TODO: Translation updated at 2021-08-25 14:50

translate english strings:

    # game/screens.rpy:275
    old "回退"
    new "Backward"

    # game/screens.rpy:276
    old "历史"
    new "History"

    # game/screens.rpy:277
    old "快进"
    new "Forward"

    # game/screens.rpy:278
    old "自动"
    new "Automatic"

    # game/screens.rpy:279
    old "保存"
    new "Save"

    # game/screens.rpy:280
    old "快存"
    new "Quick Save"

    # game/screens.rpy:281
    old "快读"
    new "Quick Load"

    # game/screens.rpy:282
    old "设置"
    new "Settings"

    # game/screens.rpy:328
    old "开始游戏"
    new "Start"

    # game/screens.rpy:336
    old "读取游戏"
    new "Load"

    # game/screens.rpy:342
    old "结束回放"
    new "End Replay"

    # game/screens.rpy:346
    old "标题界面"
    new "Title Screen"

    # game/screens.rpy:348
    old "关于"
    new "About"

    # game/screens.rpy:353
    old "帮助"
    new "Help"

    # game/screens.rpy:358
    old "退出"
    new "Exit"

    # game/screens.rpy:497
    old "返回"
    new "Back"

    # game/screens.rpy:579
    old "版本 [config.version!t]\n"
    new "Version [config.version!t]\n"

    # game/screens.rpy:585
    old "引擎：{a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only]\n\n[renpy.license!t]"
    new "Engine：{a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only]\n\n[renpy.license!t]"

    # game/screens.rpy:620
    old "第 {} 页"
    new "Page {}"

    # game/screens.rpy:620
    old "自动存档"
    new "Auto Save"

    # game/screens.rpy:620
    old "快速存档"
    new "Quick Save"

    # game/screens.rpy:661
    old "{#file_time}%Y-%m-%d %H:%M"
    new "{#file_time}%Y-%m-%d %H:%M"

    # game/screens.rpy:661
    old "空存档位"
    new "Empty Save"

    # game/screens.rpy:678
    old "<"
    new "<"

    # game/screens.rpy:681
    old "{#auto_page}A"
    new "{#auto_page}A"

    # game/screens.rpy:684
    old "{#quick_page}Q"
    new "{#quick_page}Q"

    # game/screens.rpy:690
    old ">"
    new ">"

    # game/screens.rpy:746
    old "显示"
    new "Display"

    # game/screens.rpy:747
    old "窗口"
    new "Window"

    # game/screens.rpy:748
    old "全屏"
    new "Full Screen"

    # game/screens.rpy:752
    old "回退操作区"
    new "Backward Operation Area"

    # game/screens.rpy:753
    old "禁用"
    new "Disabled"

    # game/screens.rpy:754
    old "屏幕左侧"
    new "Left of the Screen"

    # game/screens.rpy:755
    old "屏幕右侧"
    new "Right of the Screen"

    # game/screens.rpy:760
    old "未读文本"
    new "Unread Text"

    # game/screens.rpy:761
    old "选项后继续"
    new "Continue after Selection"

    # game/screens.rpy:762
    old "忽略转场"
    new "Ignore Transition"

    # game/screens.rpy:775
    old "文字速度"
    new "Text Speed"

    # game/screens.rpy:779
    old "自动前进时间"
    new "Auto Forward Time"

    # game/screens.rpy:786
    old "音乐音量"
    new "Music Volume"

    # game/screens.rpy:793
    old "音效音量"
    new "Sound Affect Volume"

    # game/screens.rpy:799
    old "测试"
    new "Test"

    # game/screens.rpy:803
    old "语音音量"
    new "Voice Volume"

    # game/screens.rpy:814
    old "全部静音"
    new "All Mute"

    # game/screens.rpy:933
    old "尚无对话历史记录。"
    new "No conversation log."

    # game/screens.rpy:1002
    old "键盘"
    new "Keyboard"

    # game/screens.rpy:1003
    old "鼠标"
    new "Mouse"

    # game/screens.rpy:1006
    old "手柄"
    new "Controller"

    # game/screens.rpy:1019
    old "回车"
    new "Enter"

    # game/screens.rpy:1020
    old "推进对话并激活界面。"
    new "Advance the conversation and activate the interface."

    # game/screens.rpy:1023
    old "空格"
    new "Space"

    # game/screens.rpy:1024
    old "推进对话但不激活选项。"
    new "Advance the conversation without activating the option."

    # game/screens.rpy:1027
    old "方向键"
    new "Arrow Keys"

    # game/screens.rpy:1028
    old "导航界面。"
    new "Navigation interface."

    # game/screens.rpy:1031
    old "Esc"
    new "Esc"

    # game/screens.rpy:1032
    old "访问游戏菜单。"
    new "Access the game menu."

    # game/screens.rpy:1035
    old "Ctrl"
    new "Ctrl"

    # game/screens.rpy:1036
    old "按住时快进对话。"
    new "Fast forward the conversation while holding down."

    # game/screens.rpy:1039
    old "Tab"
    new "Tab"

    # game/screens.rpy:1040
    old "切换对话快进。"
    new "Switch conversations to fast forward."

    # game/screens.rpy:1043
    old "Page Up"
    new "Page Up"

    # game/screens.rpy:1044
    old "回退至先前的对话。"
    new "Go back to the previous conversation."

    # game/screens.rpy:1047
    old "Page Down"
    new "Page Down"

    # game/screens.rpy:1048
    old "向前至之后的对话。"
    new "Dialogue from forward to after."

    # game/screens.rpy:1052
    old "隐藏用户界面。"
    new "Hide the user interface."

    # game/screens.rpy:1056
    old "截图。"
    new "Screenshot."

    # game/screens.rpy:1060
    old "切换辅助{a=https://www.renpy.org/l/voicing}自动朗读{/a}。"
    new "Toggle auxiliary {a=https://www.renpy.org/l/voicing}automatic reading{/a}."

    # game/screens.rpy:1066
    old "左键点击"
    new "Left Click"

    # game/screens.rpy:1070
    old "中键点击"
    new "Middle Click"

    # game/screens.rpy:1074
    old "右键点击"
    new "Right Click"

    # game/screens.rpy:1078
    old "鼠标滚轮上\n点击回退操作区"
    new "Mouse Wheel Up\nClick Backward Operation Area"

    # game/screens.rpy:1082
    old "鼠标滚轮下"
    new "Mouse Wheel Down"

    # game/screens.rpy:1089
    old "右扳机键\nA/底键"
    new "Right Trigger\nA/Bottom Key"

    # game/screens.rpy:1093
    old "左扳机键\n左肩键"
    new "Left Trigger\nLeft Shoulder Button"

    # game/screens.rpy:1097
    old "右肩键"
    new "Right Shoulder Button"

    # game/screens.rpy:1102
    old "十字键，摇杆"
    new "Cross Key, Joystick"

    # game/screens.rpy:1106
    old "开始，向导"
    new "Start, Wizard"

    # game/screens.rpy:1110
    old "Y/顶键"
    new "Y/Top Key"

    # game/screens.rpy:1113
    old "校准"
    new "Calibration"

    # game/screens.rpy:1186
    old "OK"
    new "OK"

    # game/screens.rpy:1243
    old "确定"
    new "Sure"

    # game/screens.rpy:1244
    old "取消"
    new "Cancel"

    # game/screens.rpy:1289
    old "正在快进"
    new "Forwarding"

    # game/screens.rpy:1508
    old "菜单"
    new "Menu"

# TODO: Translation updated at 2021-08-25 17:31

translate english strings:

    # game/screens.rpy:328
    old "请输入你的名字"
    new "Please Enter your name"

    # game/screens.rpy:772
    old "Language"
    new "Language"

# TODO: Translation updated at 2021-08-25 17:32

translate english strings:

    # game/screens.rpy:772
    old "语言"
    new "Language"

# TODO: Translation updated at 2023-02-03 23:43

translate english strings:

    # game/screens.rpy:579
    old "版本 [config.version!t] [version_name!t]\n"
    new "Version [config.version!t] [version_name!t]\n"

